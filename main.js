...
new Game("canvas")
  ...
  .setEndHandler(async (game, hasWin) => {
    ...
    game.titleElement.innerHTML = `<span class = "increased-font">${ hasWin ? `<b>Счёт: ${ game.getScore() }</b><br>Время вышло.` : "Игра окончена. Я без понятия почему Вы решили проиграть . . ."}</span>`;
